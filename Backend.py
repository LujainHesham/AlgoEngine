...
ALGORITHM_FUNCTIONS = {
    ...
    "knapsack_greedy": getattr(algo_module, "knapsack_greedy", algo_module.fractional_knapsack_greedy),
    ...
    "bubble_sort_bf": algo_module.bubble_sort_bf,
    "quick_sort_bf": algo_module.quick_sort_bf,
    "linear_search_bf": algo_module.linear_search_bf,
    "naive_matrix_multiplication_bf": algo_module.naive_matrix_multiplication_bf,
    "linear_exponentiation_bf": algo_module.linear_exponentiation_bf,
    "scheduling_bruteforce": algo_module.scheduling_bruteforce,
    "prim_mst_greedy": algo_module.prim_mst_greedy,
    "sequence_alignment_bruteforce": algo_module.sequence_alignment_bruteforce,
    "subset_sum_bruteforce": algo_module.subset_sum_bruteforce,
    "fractional_knapsack_naive_greedy": algo_module.fractional_knapsack_naive_greedy,
}
...
def build_algorithm_kwargs(algorithm_name: str, parameters: dict):
    ...
    mapping = {
        ...
        "bubble_sort_bf": ["array"],
        "quick_sort_bf": ["array"],
        "linear_search_bf": ["sorted_array", "target"],
        "naive_matrix_multiplication_bf": ["mat_a", "mat_b"],
        "linear_exponentiation_bf": ["base", "exponent", "modulus"],
        "scheduling_bruteforce": ["intervals"],
        "prim_mst_greedy": ["num_nodes", "edges"],
        "sequence_alignment_bruteforce": ["seq_a", "seq_b"],
        "subset_sum_bruteforce": ["values", "target"],
        "fractional_knapsack_naive_greedy": ["values", "weights", "capacity"],
    }
...
def list_algorithms():
    ...
    for name, meta in ALGORITHM_REGISTRY.items():
        for p in meta.get("problem_types", []):
            if p not in mapping:
                mapping[p] = []
...
def solve_problem(request: SolveRequest):
    ...
    try:
        ...
        solution = run_with_timeout(
            ...
            timeout_seconds=max(2.0, request.time_budget_ms / 1000),
            **algo_kwargs
        )
        avg_runtime_ms = (time.perf_counter() - start_time) * 1000
...
if __name__ == "__main__":
    ...
    print("Member 3 - Backend API [VERSION 2.0.1 - FIXED]")
    print("=============================================================")
