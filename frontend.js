...
window.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        PROBLEM_MAPPING = data.problem_mapping;
    } catch(e) {
...
function gatherInputs(type) {
    let params = {};
    try {
        if (["knapsack", "fractional_knapsack", "subset"].includes(type)) {
            const v = document.getElementById("dataInputValues").value;
            const w = document.getElementById("dataInputWeights").value;
            const c = document.getElementById("dataInputCapacity").value;
            if (v) params.values = v.split(",").map(Number);
            if (w) params.weights = w.split(",").map(Number);
            if (c) params.capacity = Number(c);
        } else if (["sorting", "searching"].includes(type)) {
            const a = document.getElementById("dataInputArray").value;
            if (a) {
                params.array = a.split(",").map(Number);
                if (type === "searching") params.sorted_array = params.array;
            }
            if (type === "searching" && a) {
                const t = document.getElementById("dataInputTarget").value;
                if (t) params.target = Number(t);
                params.is_sorted = document.getElementById("isSorted").checked;
            }
        } else if (["mst", "shortest_path"].includes(type)) {
            const e = document.getElementById("dataInputEdges").value;
            if (e) {
                params.edges = e.split(";").filter(x => x.trim() !== "").map(x => {
                    const [u, v, w] = x.split(",").map(Number);
                    return {from: u, to: v, weight: w};
                });
                let maxNode = 0;
                params.edges.forEach(edge => {
                    if (edge.from > maxNode) maxNode = edge.from;
                    if (edge.to > maxNode) maxNode = edge.to;
                });
                params.num_nodes = maxNode + 1;
                params.adjacency = Array(params.num_nodes).fill(null).map(() => []);
                params.edges.forEach(edge => {
                    params.adjacency[edge.from].push({to: edge.to, weight: edge.weight});
                    if (type === "mst") {
                        params.adjacency[edge.to].push({to: edge.from, weight: edge.weight});
                    }
                });
                params.source_node = 0;
                if (type === "shortest_path") {
                    params.has_negative_weights = document.getElementById("hasNegativeWeights").checked;
                }
            }
        } else if (type === "sequence_alignment") {
            const a = document.getElementById("dataInputSeqA").value;
            const b = document.getElementById("dataInputSeqB").value;
            if (a || b) {
                if (a) params.seq_a = a.trim();
                if (b) params.seq_b = b.trim();
                params.gap_penalty = 1;
                params.mismatch_penalty = 1;
            }
        } else if (type === "exponentiation") {
            const b = document.getElementById("dataInputBase").value;
            const e = document.getElementById("dataInputExponent").value;
            if (b || e) {
                if (b) params.base = Number(b);
                if (e) params.exponent = Number(e);
                params.modulus = 1000000007;
            }
        } else if (type === "matrix_mult") {
            const a = document.getElementById("dataInputMatrixA").value;
            const b = document.getElementById("dataInputMatrixB").value;
            if (a) params.mat_a = a.split(";").filter(x => x.trim() !== "").map(row => row.split(",").map(Number));
            if (b) params.mat_b = b.split(";").filter(x => x.trim() !== "").map(row => row.split(",").map(Number));
        } else if (type === "scheduling") {
            const ivals = document.getElementById("dataInputIntervals").value;
            if (ivals) {
                params.intervals = ivals.split(";").filter(x => x.trim() !== "").map(x => {
                    const [s, e, w] = x.split(",").map(Number);
                    return [s, e, w];
                });
            }
        }
    } catch(e) { console.warn("Error parsing inputs", e); }
...
async function callSolveApi(type, n, T, quality, parameters = {}) {
    try {
...
async function callCompareApi(type, n, algorithms, parameters = {}) {
    try {
...
function getComplexityMetrics(algorithmName) {
    const registry = {
        ...
        'matrix_multiplication_dc': { time: "O(n³)", space: "O(n²)", quality: "100% Optimal" },
        'bubble_sort_bf': { time: "O(n²)", space: "O(1)", quality: "100% Optimal" },
        'quick_sort_bf': { time: "O(n log n)", space: "O(log n)", quality: "100% Optimal" },
        'linear_search_bf': { time: "O(n)", space: "O(1)", quality: "100% Optimal" },
        'naive_matrix_multiplication_bf': { time: "O(n³)", space: "O(n²)", quality: "100% Optimal" },
        'linear_exponentiation_bf': { time: "O(n)", space: "O(1)", quality: "100% Optimal" },
        'scheduling_bruteforce': { time: "O(2^n)", space: "O(n)", quality: "100% Optimal" },
        'prim_mst_greedy': { time: "O(E log V)", space: "O(V + E)", quality: "100% Optimal" },
        'sequence_alignment_bruteforce': { time: "O(3^n)", space: "O(n)", quality: "100% Optimal" },
        'subset_sum_bruteforce': { time: "O(2^n)", space: "O(n)", quality: "100% Optimal" },
        'fractional_knapsack_naive_greedy': { time: "O(n log n)", space: "O(n)", quality: "~70% Approx" }
    };
    ...
    try {
        if (type === 'knapsack' || type === 'fractional_knapsack') {
            solutionHTML += `<div class="result-row">
                ...
                <span class="result-row-value">${solution.total_value !== undefined ? solution.total_value : 'N/A'}</span>
            </div>`;
            ...
            solutionHTML += `<div class="result-row">
                <span class="result-row-label">Edit Distance (Alignment Cost)</span>
                <span class="result-row-value">${solution.edit_distance !== undefined ? solution.edit_distance : '?'}</span>
            </div>`;
            solutionHTML += `<div class="result-row">
...
async function runExperiment(type, n, T, quality) {
    ...
    let algorithmsToRun = PROBLEM_MAPPING[type] || [];
    ...
    try {
        ...
        const compareResult = await callCompareApi(type, n, algorithmsToRun, params);
        ...
        // compute approximation ratios - use the most meaningful value per problem type
        ...
        results.forEach(r => {
            ...
            let val = null;
            if (sol) {
                 if (sol.total_value !== undefined) val = sol.total_value;           // knapsack, fractional
                 else if (sol.edit_distance !== undefined) val = sol.edit_distance;  // sequence alignment
                 else if (sol.total_weight !== undefined) val = sol.total_weight;    // MST, scheduling
                 else if (sol.result !== undefined && typeof sol.result === 'number') val = sol.result;  // exponentiation
                 else if (sol.found !== undefined) val = sol.found ? 1 : 0;         // binary search
                 else if (sol.distances !== undefined && Array.isArray(sol.distances)) {
                     val = sol.distances.filter(d => d !== Infinity && d !== null).reduce((a, b) => a + b, 0);
                 }
                 else val = 1;
            }
        ...
        closeBtn.onclick = () => {
            // Show main content, hide experiment section
...
async function runBenchmark(type) {
    ...
    else if (type === 'sorting') algorithms = ['merge_sort_dc', 'bubble_sort_bf', 'quick_sort_bf'];
    else if (type === 'mst') algorithms = ['kruskal_mst_greedy', 'prim_mst_greedy'];
    else if (type === 'matrix_mult') algorithms = ['matrix_multiplication_dc', 'naive_matrix_multiplication_bf'];
    else if (type === 'searching') algorithms = ['binary_search_dc', 'linear_search_bf'];
    else if (type === 'scheduling') algorithms = ['weighted_interval_scheduling_dp', 'scheduling_bruteforce'];
    else if (type === 'exponentiation') algorithms = ['fast_exponentiation_dc', 'linear_exponentiation_bf'];
    else if (type === 'sequence_alignment') algorithms = ['sequence_alignment_dp', 'sequence_alignment_bruteforce'];
    else if (type === 'fractional_knapsack') algorithms = ['fractional_knapsack_greedy', 'fractional_knapsack_naive_greedy'];
    else if (type === 'subset') algorithms = ['subset_bruteforce', 'subset_sum_bruteforce'];
...
document.getElementById('sizeSlider').addEventListener('input', (e) => {
    ...
    if (value > 15 && (type === 'subset')) {
        warning = '<div class="danger-badge">⚠ Subset enumeration: 2^n = ' + Math.pow(2, value).toLocaleString() + ' states — infeasible!</div>';
    } else if (value <= 15 && (type === 'knapsack' || type === 'subset')) {
        warning = '<div class="warning-badge">✓ n ≤ 15: Brute Force eligible (2^' + value + ' = ' + Math.pow(2, value).toLocaleString() + ' states)</div>';
    } else if (value > 15 && type === 'knapsack') {
        warning = '<div class="warning-badge">⚠ n > 15: Brute Force disabled — DP or Greedy will be used</div>';
    }
...
document.getElementById('timeSlider').addEventListener('input', (e) => {
    ...
    let badge = '';
    if (value < 100) {
        badge = '<span class="time-budget-indicator critical">✗ Critical — Greedy preferred</span>';
    } else if (value <= 500) {
        badge = '<span class="time-budget-indicator warning">⚠ Moderate time budget</span>';
    } else {
        badge = '<span class="time-budget-indicator good">✓ Ample — all algorithms eligible</span>';
    }
...
document.getElementById('solveBtn').addEventListener('click', async () => {
    ...
    try {
        const params = gatherInputs(type);
        const result = await callSolveApi(type, n, T, quality, params);
...
document.getElementById('experimentBtn').addEventListener('click', () => {
    ...
    // Hide main content, show experiment section
    document.getElementById('mainContent').classList.add('hidden');
    document.getElementById('experimentSection').classList.add('active');
...
document.getElementById('problemType').addEventListener('change', (e) => {
    ...
    const inputs = ['dataInputValues', 'dataInputWeights', 'dataInputCapacity', 'dataInputArray', 'isSortedContainer', 'dataInputTarget', 'dataInputEdges', 'hasNegativeWeightsContainer', 'dataInputSeqA', 'dataInputSeqB', 'dataInputBase', 'dataInputExponent', 'dataInputMatrixA', 'dataInputMatrixB', 'dataInputIntervals'];
    inputs.forEach(id => {
    ...
    if (['knapsack', 'fractional_knapsack', 'subset'].includes(type)) {
        document.getElementById('dataInputValues').style.display = 'block';
        document.getElementById('dataInputWeights').style.display = 'block';
        document.getElementById('dataInputCapacity').style.display = 'block';
    } else if (['sorting', 'searching'].includes(type)) {
        document.getElementById('dataInputArray').style.display = 'block';
        if (type === 'searching') {
            document.getElementById('isSortedContainer').style.display = 'flex';
            document.getElementById('dataInputTarget').style.display = 'block';
        }
    } else if (['mst', 'shortest_path'].includes(type)) {
        document.getElementById('dataInputEdges').style.display = 'block';
        if (type === 'shortest_path') {
            document.getElementById('hasNegativeWeightsContainer').style.display = 'flex';
        }
    } else if (type === 'sequence_alignment') {
        document.getElementById('dataInputSeqA').style.display = 'block';
        document.getElementById('dataInputSeqB').style.display = 'block';
    } else if (type === 'exponentiation') {
        document.getElementById('dataInputBase').style.display = 'block';
        document.getElementById('dataInputExponent').style.display = 'block';
    } else if (type === 'matrix_mult') {
        document.getElementById('dataInputMatrixA').style.display = 'block';
        document.getElementById('dataInputMatrixB').style.display = 'block';
    } else if (type === 'scheduling') {
        document.getElementById('dataInputIntervals').style.display = 'block';
    }
});
