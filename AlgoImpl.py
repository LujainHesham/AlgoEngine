...
def matrix_multiplication_dc(mat_a: list, mat_b: list) -> dict:
    """
    Multiplies two n x n matrices using a standard Divide & Conquer approach.
    Recursively splits matrices into quadrants. Uses 8 recursive multiplications
    per level → O(n³). This is NOT Strassen's algorithm (which uses 7).
    
    ⚠️ IMPORTANT: This implementation requires n to be a POWER OF 2.
    For non-power-of-2 matrices, the dataset loader automatically pads to the next power of 2.
    If you get incorrect results, check that your matrix dimensions are powers of 2.
    
    Args:
        mat_a : 2D list (n x n) where n is a power of 2
        mat_b : 2D list (n x n) where n is a power of 2
...
def sequence_alignment_dp(seq_a: str, seq_b: str, gap_penalty: int = 1,
    ...
    return {
        ...
        "score": edit_distance,  # ← ADD THIS for frontend compatibility
...
def weighted_interval_scheduling_dp(intervals: list) -> dict:
    ...
    return {
        ...
        "max_weight": dp_table[num_intervals],
        "dp_table"          : dp_table,
        "backtrack_sequence": backtrack_sequence,
        "runtime_ms"        : round(runtime_ms, 4),
    }
...
def knapsack_greedy(values: list, weights: list, capacity: int) -> dict:
    """
    Solves the 0/1 Knapsack problem using a Greedy approach.
    Strategy: sort by value-per-unit-weight and take whole items.
    
    ⚠️ WARNING: This is an approximation with NO guaranteed bound.
    In worst case, greedy can be arbitrarily far from optimal.
    """
    start_time = time.perf_counter()
    ...
    # Find the single best item (for a better lower bound)
    best_single_value = max(values) if values else 0
    
    return {
        "selected_items": sorted(selected_items),
        "total_value": total_value,
        "total_weight": total_weight,
        "approximation_ratio": None,  # No guaranteed bound for 0/1 knapsack greedy
        "best_possible_upper_bound": None,  # Cannot compute without optimal
        "warning_message": "⚠️ 0/1 Knapsack greedy is an approximation with NO guaranteed bound. In worst case, it can be arbitrarily far from optimal. Use DP for exact solution when possible.",
        "runtime_ms": round(runtime_ms, 4),
    }

# ─────────────────────────────────────────────────────────────

def fractional_knapsack_greedy(values: list, weights: list, capacity: int) -> dict:
    """
    ...
    Returns:
        dict with keys:
          ...
          approximation_ratio– 1.0 (greedy IS optimal for fractional)
...
def binary_search_dc(sorted_array: list, target) -> dict:
    ...
    level_sizes    = []
    comparisons    = [0]    # ← ADD THIS - counter for comparisons
    ...
    def _binary_search(low: int, high: int, depth: int) -> int:
        ...
        mid = (low + high) // 2
        comparisons[0] += 1  # ← ADD THIS - count this comparison
...
def fast_exponentiation_dc(base: int, exponent: int,
    ...
    Returns:
        dict with keys:
          ...
          multiplications– number of multiplications performed
    ...
    level_sizes    = []
    max_depth_seen = [0]
    mult_count     = [0]  # ← ADD THIS - count multiplications
    ...
    def _fast_exp(current_base: int, current_exp: int, depth: int) -> int:
        ...
        if current_exp % 2 == 0:
            ...
            mult_count[0] += 1  # ← ADD THIS - one multiplication
        else:
            result_val = current_base * half * half
            mult_count[0] += 2  # ← ADD THIS - two multiplications
    ...
    return {
        "result"         : final_result,
        "multiplications": mult_count[0],  # ← ADD THIS
        "recursion_depth": max_depth_seen[0],
        "level_sizes"    : level_sizes,
        "runtime_ms"     : round(runtime_ms, 4),
    }
...
# ── Comparison & Baseline Algorithms (for Experiment Mode) ────

def bubble_sort_bf(array: list) -> dict:
    """Naive bubble sort (O(n^2)) for comparison."""
    start_time = time.perf_counter()
    arr = array[:]
    n = len(arr)
    for i in range(n):
    ...
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"sorted_array": arr, "runtime_ms": round(runtime_ms, 4)}

def quick_sort_bf(array: list) -> dict:
    """Quick Sort (O(n log n)) for comparison."""
    start_time = time.perf_counter()
    arr = array[:]
    ...
    res = _quick_sort(arr)
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"sorted_array": res, "runtime_ms": round(runtime_ms, 4)}

def linear_search_bf(sorted_array: list, target: int) -> dict:
    """Naive linear search (O(n)) for comparison."""
    start_time = time.perf_counter()
    found = False
    index = -1
    for i, val in enumerate(sorted_array):
        if val == target:
            ...
            break
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"found": found, "index": index, "runtime_ms": round(runtime_ms, 4)}

def naive_matrix_multiplication_bf(mat_a: list, mat_b: list) -> dict:
    """Naive O(n^3) matrix multiplication for comparison."""
    start_time = time.perf_counter()
    n = len(mat_a)
    ...
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"result": result, "runtime_ms": round(runtime_ms, 4)}

def linear_exponentiation_bf(base: int, exponent: int, modulus: int) -> dict:
    """Naive O(n) exponentiation for comparison."""
    start_time = time.perf_counter()
    result = 1
    # Limit exponent for safety
    safe_exponent = min(exponent, 100000)
    for _ in range(safe_exponent):
        result = (result * base) % modulus
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"result": result, "runtime_ms": round(runtime_ms, 4)}


def scheduling_bruteforce(intervals: list) -> dict:
    """Naive recursive scheduling for comparison (limited n)."""
    start_time = time.perf_counter()
    # Sort by end time anyway to simplify
    sorted_ivals = sorted(intervals, key=lambda x: x[1])
    n = len(sorted_ivals)
    
    # We use memoization here but it's still "brute force style" recursion
    memo = {}
    def solve(idx):
        if idx < 0: return 0
        if idx in memo: return memo[idx]
        # Include
        incl = sorted_ivals[idx][2]
        next_idx = -1
        for j in range(idx - 1, -1, -1):
            if sorted_ivals[j][1] <= sorted_ivals[idx][0]:
                next_idx = j
                break
        incl += solve(next_idx)
        # Exclude
    ...
    res = solve(n - 1)
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"total_weight": res, "runtime_ms": round(runtime_ms, 4)}


def prim_mst_greedy(num_nodes: int, edges: list) -> dict:
    """Prim's algorithm for MST (O(V^2)) for comparison."""
    import heapq
    start_time = time.perf_counter()
    ...
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"mst_edges": mst_edges, "total_weight": total_weight, "runtime_ms": round(runtime_ms, 4)}

def sequence_alignment_bruteforce(seq_a: str, seq_b: str) -> dict:
    """Brute force recursive sequence alignment (O(3^n)) for comparison."""
    start_time = time.perf_counter()
    
    def align(i, j):
        if i == len(seq_a): return (len(seq_b) - j) * -1
        if j == len(seq_b): return (len(seq_a) - i) * -1
        
        match = align(i+1, j+1) + (1 if seq_a[i] == seq_b[j] else -1)
        gap_a = align(i+1, j) - 1
        gap_b = align(i, j+1) - 1
        return max(match, gap_a, gap_b)
    
    # Safety limit
    if len(seq_a) > 10 or len(seq_b) > 10:
        res = -999 # dummy for large n
    else:
        res = align(0, 0)
        
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"score": res, "runtime_ms": round(runtime_ms, 4)}

def subset_sum_bruteforce(values: list, target: int) -> dict:
    """Pure recursive subset sum (O(2^n)) for comparison."""
    start_time = time.perf_counter()
    n = len(values)
    ...
    def solve(idx, current_sum):
        ...
        # Include
        if solve(idx + 1, current_sum + values[idx]): return True
        # Exclude
    ...
    found = solve(0, 0)
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"found": found, "runtime_ms": round(runtime_ms, 4)}

def fractional_knapsack_naive_greedy(values: list, weights: list, capacity: int) -> dict:
    """Naive greedy (by weight) for fractional knapsack (sub-optimal)."""
    start_time = time.perf_counter()
    items = []
    for i in range(len(values)):
        items.append({"id": i, "v": values[i], "w": weights[i]})
    
    # Sort by weight (ascending) - naive logic: "take lighter things first"
    ...
    for item in items:
        ...
        if item["w"] <= rem:
            ...
            fractions[item["id"]] = rem / item["w"]
            total_value += item["v"] * (rem / item["w"])
            rem = 0
            
    runtime_ms = (time.perf_counter() - start_time) * 1000
    return {"total_value": total_value, "fractions": fractions, "runtime_ms": round(runtime_ms, 4)}
...
ALGORITHM_REGISTRY: dict = {
    ...
    "knapsack_greedy": {
        ...
        "space_complexity" : "O(n)",
        "quality"          : "approximate",
        "approx_ratio"     : None,  # ← FIXED: No guaranteed bound
        "approx_guaranteed": False,
        "thresholds"       : {
            "max_n"               : 10_000_000,
        },
        "warning"          : "⚠️ 0/1 Knapsack greedy has NO guaranteed approximation bound. In worst case, it can be arbitrarily far from optimal. Use DP for exact solutions.",
        "callable_name"    : "knapsack_greedy",
    },
    ...
    "bubble_sort_bf": {
        ...
        "approx_ratio": 1.0,
        "approx_guaranteed": True,
        "thresholds": {"max_n": 1000},
        "warning": "Bubble sort is inefficient for large n.",
        "callable_name": "bubble_sort_bf",
    },
    "quick_sort_bf": {
        "family": "divide_and_conquer",
        "problem_types": ["sorting"],
        ...
        "approx_ratio": 1.0,
        "approx_guaranteed": True,
        "thresholds": {"max_n": 100000},
        "warning": None,
        "callable_name": "quick_sort_bf",
    },
    "linear_search_bf": {
        "family": "brute_force",
        "problem_types": ["searching"],
        ...
        "approx_ratio": 1.0,
        "approx_guaranteed": True,
        "thresholds": {"max_n": 1000000},
        "warning": "Linear search is slower than binary search for sorted data.",
        "callable_name": "linear_search_bf",
    },
    "naive_matrix_multiplication_bf": {
        "family": "brute_force",
        "problem_types": ["matrix_mult"],
        "time_complexity": "O(n³)",
        "space_complexity": "O(n²)",
        ...
        "approx_ratio": 1.0,
        "approx_guaranteed": True,
        "thresholds": {"max_n": 128},
        "warning": "Naive matrix multiplication has the same O(n³) as the D&C version but with higher constant factors.",
        "callable_name": "naive_matrix_multiplication_bf",
    },
    "linear_exponentiation_bf": {
        "family": "brute_force",
        "problem_types": ["exponentiation"],
        "time_complexity": "O(n)",
        "space_complexity": "O(1)",
        ...
        "approx_guaranteed": True,
        "thresholds": {"max_n": 100000},
        "warning": "Linear exponentiation is much slower than fast exponentiation.",
        "callable_name": "linear_exponentiation_bf",
    },
    "scheduling_bruteforce": {
        "family": "brute_force",
        "problem_types": ["scheduling"],
        "time_complexity": "O(2^n)",
        "space_complexity": "O(n)",
        ...
        "approx_guaranteed": True,
        "thresholds": {"max_n": 20},
        "warning": "Brute force scheduling is exponential.",
        "callable_name": "scheduling_bruteforce",
    },
    "fractional_knapsack_naive_greedy": {
        "family": "greedy",
        "problem_types": ["fractional_knapsack"],
        "time_complexity": "O(n log n)",
        "space_complexity": "O(n)",
        ...
        "approx_guaranteed": False,
        "thresholds": {"max_n": 1000000},
        "warning": "Naive greedy (by weight) is sub-optimal for fractional knapsack.",
        "callable_name": "fractional_knapsack_naive_greedy",
    },
    "prim_mst_greedy": {
        "family": "greedy",
        "problem_types": ["mst"],
        ...
        "thresholds": {"max_n": 100000},
        "warning": None,
        "callable_name": "prim_mst_greedy",
    },
    "sequence_alignment_bruteforce": {
        "family": "brute_force",
        "problem_types": ["sequence_alignment"],
        ...
        "approx_guaranteed": True,
        "thresholds": {"max_n": 10},
        "warning": "Recursive alignment is extremely slow.",
        "callable_name": "sequence_alignment_bruteforce",
    },
    "subset_sum_bruteforce": {
        ...
        "approx_guaranteed": True,
        "thresholds": {"max_n": 20},
        "warning": "Brute force subset sum is exponential.",
        "callable_name": "subset_sum_bruteforce",
    },
}
...
def run_algorithm(algorithm_name: str, **kwargs) -> dict:
    ...
    _DISPATCH_TABLE: dict = {
        ...
        "bubble_sort_bf"                 : bubble_sort_bf,
        "quick_sort_bf"                  : quick_sort_bf,
        "linear_search_bf"               : linear_search_bf,
        "naive_matrix_multiplication_bf" : naive_matrix_multiplication_bf,
        "linear_exponentiation_bf"       : linear_exponentiation_bf,
        "scheduling_bruteforce"          : scheduling_bruteforce,
        "prim_mst_greedy"                : prim_mst_greedy,
        "sequence_alignment_bruteforce"  : sequence_alignment_bruteforce,
        "subset_sum_bruteforce"          : subset_sum_bruteforce,
        "fractional_knapsack_naive_greedy": fractional_knapsack_naive_greedy,
    }
...
def run_experiment_mode(problem_type: str, n: int, **kwargs) -> dict:
    ...
    # Helper function to handle None values in approx_ratio
    def get_ratio(metadata):
        ratio = metadata.get("approx_ratio")
        if ratio is None:
            return -1.0  # Put None values at the bottom of ranking
        return ratio

    # Sort: highest approximation ratio first, then fastest runtime
    results.sort(
        key=lambda r: (
            -get_ratio(r["metadata"]),   # descending ratio (best first)
...
if __name__ == "__main__":
    ...
    # Test 1: knapsack_greedy
    result = knapsack_greedy([60, 100, 120], [10, 20, 30], 50)
    print("knapsack_greedy approx_ratio:", result["approximation_ratio"])  # Should be None

    # Test 2: weighted_interval_scheduling_dp
    result = weighted_interval_scheduling_dp([(1,3,5), (2,5,6), (4,6,5)])
    print("Has max_weight:", "max_weight" in result)  # Should be True

    # Test 3: sequence_alignment_dp
    result = sequence_alignment_dp("ACGT", "AGT")
    print("Has score:", "score" in result)  # Should be True

    # Test 4: binary_search_dc
    result = binary_search_dc([1,3,5,7,9], 5)
    print("Has steps:", "steps" in result)  # Should be True
    print("Steps count:", result["steps"])  # Should be 1 or 2

    # Test 5: fast_exponentiation_dc
    result = fast_exponentiation_dc(2, 10)
    print("Has multiplications:", "multiplications" in result)  # Should be True
