    ...
    if problem_type == "matrix_mult":
        ...
        if n > meta["thresholds"]["max_n"]:
            raise ValueError(
                ...
                f"({meta['thresholds']['max_n']}) for D&C matrix multiplication. "
                "The recursion depth would be too great."
            )
        return _build_result(
            algorithm_name = "matrix_multiplication_dc",
            justification  = (
                "Divide & Conquer (Recursive Matrix Multiplication) selected. "
                "Matrix multiplication can be solved recursively by splitting matrices "
                "into quadrants. This simple implementation uses 8 multiplications "
                "for an O(n³) runtime. "
                f"For n={n}, this approach is straightforward."
            ),
            meta     = meta,
            warnings = warnings,
        )
    ...
    if problem_type in ("knapsack", "fractional_knapsack"):
        ...
        if quality_requirement in ("approximate", "best_effort") or time_budget_ms < 100:
            if problem_type == "knapsack":
                meta = ALGORITHM_REGISTRY["knapsack_greedy"]
                ...
                warnings.append(
                    "0/1 Knapsack greedy used as approximation. "
                    "Note: this is not optimal."
                )
                return _build_result(
                    algorithm_name = "knapsack_greedy",
                    justification  = (
                        f"Greedy (0/1 Knapsack) selected. {reason} "
                ...
                return _build_result(
                    ...
                    justification  = (
                        ...
                        "relaxation, this IS optimal (ratio=1.0)."
                    ),
                    meta     = meta,
                    warnings = warnings,
                )
        ...
        return _build_result(
            algorithm_name = "knapsack_greedy",
            justification  = (
                "Greedy (0/1 Knapsack) selected as fallback. "
